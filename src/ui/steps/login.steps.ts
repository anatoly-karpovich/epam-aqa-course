...
class LoginSteps {
  ...
  async login(credentials?: ICredentials) {
    await LoginPage.setValue(LoginPage["Login input"], credentials?.login || config.credentials.login);
    await LoginPage.setValue(LoginPage["Password input"], credentials?.password || config.credentials.password, { isSecretValue: true });
